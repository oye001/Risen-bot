...
// Silence breaker captions (3-5 lines each)
...
// Track last N AI replies to avoid repeating
...
function recordReply(reply) {
  ...
  if (state.lastAiReplies.length > 10) state.lastAiReplies.shift();
  saveState(state);
}
      ...
      messages: [
        {
          ...
          content:
            ...
            "Every welcome must feel different and human.",
        },
        {
          role: "user",
...
bot.on("message", async (ctx) => {
  ...
  const ignoreWords = new RegExp(
    ...
    "glory|amen|hallelujah|praise|gm fam|lets go|let's go)$",
    "i"
  );
  ...
  // AI reply
  try {
    const recentReplies = (state.lastAiReplies || []).slice(-5).join("\n---\n");
      ...
      messages: [
        {
          ...
          content:
            ...
            "The developer is ACTIVE and engaged with the community every day.\n" +
            ...
            "CRITICAL: Every single reply must be completely unique. Never repeat phrasing, structure, or wording from previous replies.\n" +
            "These are your recent replies, do NOT repeat or closely echo any of them:\n" +
            recentReplies + "\n" +
...
app.listen(PORT, () => {
  ...
  // Delay webhook setup by 2 seconds to ensure server is fully ready
  setTimeout(async () => {
    const webhookTarget = WEBHOOK_URL + "/webhook";
    console.log("Setting webhook to: " + webhookTarget);
    let attempts = 0;
    const trySet = async () => {
      try {
        await bot.telegram.setWebhook(webhookTarget);
        console.log("Webhook set successfully: " + webhookTarget);
      } catch (e) {
        attempts++;
        ...
        if (attempts < 5) {
          ...
          console.error("Webhook could not be set after 5 attempts.");
        }
      }
    };
    await trySet();
